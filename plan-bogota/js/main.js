...
const filterPlans = (
  ...
  if (grid) {
    ...
    grid.innerHTML = ``;
    let url = `get/filterplans.php?filter=1&lang=${actualLang}`;
...
function get_alias(str) {
  str = str.replace("#039;", "", str); //Signo de exclamación abierta.&iexcl;
  str = str.replace("'", "", str); //Signo de exclamación abierta.&iexcl;
        ...
        $("#planForm").ajaxSubmit({
          ...
          success: function (data) {
            if (data.message == 1) {
              ...
              $('#planForm button[type="submit"]').html("RESERVA AHORA");
              $.fancybox.open({
